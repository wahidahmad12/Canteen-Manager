...
import { ScanLine, Upload, Loader2, Trash2, Save, X, ReceiptText, FileSpreadsheet } from "lucide-react";
...
export default function GroceryScanTab() {
  ...
  // Month filter for the saved-expenses report ("" = all months)
  const [filterMonth, setFilterMonth] = useState(format(new Date(), "yyyy-MM"));
  const filteredExpenses = useMemo(
    () => filterMonth ? expenses.filter(e => e.entryDate.startsWith(filterMonth)) : expenses,
    [expenses, filterMonth],
  );
  const grandTotal = useMemo(() => filteredExpenses.reduce((s, e) => s + (Number(e.cost) || 0), 0), [filteredExpenses]);

  const monthLabel = filterMonth
    ? format(new Date(filterMonth + "-01T00:00:00"), "MMMM yyyy")
    : "All Months";

  const [exporting, setExporting] = useState(false);
  const handleExportExcel = async () => {
    if (!filteredExpenses.length) { toast({ title: "No grocery expenses to export for this month", variant: "destructive" }); return; }
    setExporting(true);
    try {
      const ExcelJS = (await import("exceljs")).default;
      const wb = new ExcelJS.Workbook();
      const ws = wb.addWorksheet("Grocery Expenses");
      const thin = { top: { style: "thin" as const }, bottom: { style: "thin" as const }, left: { style: "thin" as const }, right: { style: "thin" as const } };
      const HEADERS = ["Date", "Item", "Qty", "Cost (₹)", "Paid By"];
      // Title
      ws.mergeCells(1, 1, 1, HEADERS.length);
      const title = ws.getCell("A1");
      title.value = `DJ Hospitality — Grocery Expenses — ${monthLabel}`;
      ...
      ws.columns = [{ width: 14 }, { width: 30 }, { width: 12 }, { width: 14 }, { width: 18 }];
      // Rows (oldest first for accountants)
      const sorted = [...filteredExpenses].sort((a, b) => a.entryDate.localeCompare(b.entryDate) || a.id - b.id);
      sorted.forEach(e => {
        const row = ws.addRow([
          format(new Date(e.entryDate + "T00:00:00"), "dd-MM-yyyy"),
          e.itemName,
          e.quantity || "",
          Number(e.cost) || 0,
          e.payer || "",
      ...
      // Total row
      const totRow = ws.addRow(["", "", "TOTAL", grandTotal, ""]);
      totRow.eachCell(cell => { cell.font = { bold: true }; cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFF2CC" } }; cell.border = thin; });
      ...
      a.download = `Grocery_Expenses_${filterMonth ? monthLabel.replace(" ", "_") : "All_Months"}.xlsx`;
      a.click();
      ...
      <Card className="border-0 shadow-lg overflow-hidden">
        <CardHeader className="pb-3 pt-4 bg-gradient-to-r from-teal-600 to-cyan-600 text-white">
          <CardTitle className="text-base flex items-center justify-between">
            ...
            <span className="text-xs font-normal">Total ({monthLabel}): <span className="font-mono font-bold">₹{grandTotal.toFixed(2)}</span></span>
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <div className="flex flex-wrap items-end gap-2 p-3 border-b bg-muted/30">
            <div>
              ...
              <Input type="month" value={filterMonth} onChange={e => setFilterMonth(e.target.value)} className="h-9 w-44" data-testid="input-grocery-filter-month" />
            </div>
            {filterMonth && (
              <Button type="button" variant="outline" className="h-9" onClick={() => setFilterMonth("")} data-testid="button-grocery-all-months">
                <X className="w-4 h-4 mr-1" /> All months
              </Button>
            )}
            <div className="flex-1" />
            <Button type="button" onClick={handleExportExcel} disabled={exporting || isLoading} className="h-9 bg-teal-600 hover:bg-teal-700" data-testid="button-grocery-export-excel">
              {exporting ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <FileSpreadsheet className="w-4 h-4 mr-1" />} Download Excel
            </Button>
          </div>
          {isLoading ? (
            ...
            <div className="py-10 text-center text-muted-foreground text-sm">
              {expenses.length === 0 ? "No grocery expenses yet — upload a receipt photo above." : `No grocery expenses in ${monthLabel}.`}
            </div>
          ) : (
