...
function QtyCalculator({ itemName, uom, current, onConfirm, onClose }: {
  itemName: string; uom: string; current: number;
  ...
  const press = (key: string) => {
    if (key === "C") { setExpr(""); setJustEvaled(false); return; }
    if (key === "⌫") { setExpr(e => e.slice(0, -1)); setJustEvaled(false); return; }
    if (key === "=") {
      const r = safeEval(expr);
      if (r !== null) { setExpr(String(r)); setJustEvaled(true); }
      return;
    }
    const isOp = ["÷", "×", "-", "+"].includes(key);
    if (justEvaled && !isOp) { setExpr(key); setJustEvaled(false); return; }
    setJustEvaled(false);
    setExpr(e => e + key);
  };

  const confirm = () => {
    const r = safeEval(expr) ?? (Number(expr) || 0);
    onConfirm(r);
    onClose();
  };

  const display = expr || "0";
  const preview = expr && !justEvaled ? safeEval(expr) : null;
  ...
  const btnClass = (key: string) => {
    if (["÷","×","-","+"].includes(key)) return "bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-700 font-bold text-lg h-12 rounded-xl hover:bg-amber-100 dark:hover:bg-amber-800/40 transition-colors";
    if (key === "⌫") return "bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-700 font-bold text-base h-12 rounded-xl hover:bg-red-100 dark:hover:bg-red-800/40 transition-colors";
    if (key === "C") return "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-600 font-bold text-sm h-12 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors";
    if (key === "=") return "bg-blue-600 text-white font-bold text-xl h-12 rounded-xl hover:bg-blue-700 transition-colors shadow";
    return "bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-slate-100 font-semibold text-lg h-12 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors";
  };
    ...
    <div className="p-4 space-y-3 w-72">
      {itemName && (
        <div className="text-xs text-muted-foreground font-medium truncate">
          {itemName} <span className="text-primary">({uom})</span>
        </div>
      )}
      {/* Display */}
      <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-3 text-right border border-slate-200 dark:border-slate-600 min-h-[64px]">
        <div className="text-xs text-muted-foreground h-4">{preview !== null ? `= ${preview}` : ""}</div>
        <div className="text-2xl font-mono font-bold text-slate-800 dark:text-slate-100 break-all">{display}</div>
      </div>
      {/* Keypad */}
      <div className="space-y-2">
        {ROWS.map((row, ri) => (
          <div key={ri} className="grid grid-cols-4 gap-2">
            {row.map(key => (
              <button key={key} onClick={() => press(key)} className={btnClass(key)}>{key}</button>
            ))}
          </div>
        ))}
        <div className="grid grid-cols-4 gap-2">
          <button onClick={() => press("C")} className={btnClass("C")}>C</button>
          <button onClick={() => press("=")} className={`${btnClass("=")} col-span-2`}>=</button>
          <button
            onClick={confirm}
            className="bg-green-600 text-white font-bold text-sm h-12 rounded-xl hover:bg-green-700 transition-colors shadow col-span-1"
          >OK</button>
        </div>
      </div>
    </div>
  ...
  return (
    <Layout>
      ...
      <Dialog open={calcOpen} onOpenChange={setCalcOpen}>
        <DialogContent className="p-0 w-auto max-w-[320px]">
          <DialogHeader className="px-4 pt-4 pb-0">
            <DialogTitle className="text-base flex items-center gap-2">
              <Calculator className="w-4 h-4 text-primary" /> Qty Calculator
            </DialogTitle>
          </DialogHeader>
          <QtyCalculator
            itemName={items[calcIndex]?.itemName || ""}
            uom={items[calcIndex]?.uom || ""}
            current={items[calcIndex]?.qty || 0}
            onConfirm={(v) => updateItem(calcIndex, "qty", v)}
