...
export async function registerRoutes(
  ...
  // Temporary migration endpoint - copies old production DB to Google Cloud
  app.get("/api/migrate-production-data", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!oldDbUrl || !googleDbUrl) {
        return res.status(500).json({ error: "Missing database URLs" });
      }
      ...
      for (const table of orderedTables) {
        try {
          ...
          results[table] = `${transferred}/${sourceData.rows.length} rows transferred`;
        } catch (e: any) {
