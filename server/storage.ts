...
export class DatabaseStorage implements IStorage {
  ...
  async getHulSpecialOrdersByYear(year: number): Promise<HulSpecialOrder[]> {
    const [rows] = await db.execute(sql`
      SELECT id, \`month\`, \`year\`, sl_no as slNo, date_of_supply as dateOfSupply,
             particulars, qty, rate_per_plate as ratePerPlate, total,
             created_at as createdAt, updated_at as updatedAt
      FROM hul_special_orders
      WHERE \`year\` = ${year}
      ORDER BY \`month\`, sl_no
    `);
    return rows as HulSpecialOrder[];
  }
  ...
  async getQuotations(clientScope?: string): Promise<any[]> {
    const [qRows] = await db.execute(sql`
      SELECT q.id, q.quotation_no AS quotationNo, q.quotation_date AS quotationDate, q.quotation_thru AS quotationThru,
             ...
             q.quotation_type AS quotationType, q.gst_percent AS gstPercent, q.service_charge_percent AS serviceChargePercent, q.subject, q.attention, q.salutation,
  ...
  private normalizeQuotationInput(data: any): { quotationDate: string; quotationThru: string; clientName: string; remarks: string; subject: string; attention: string; salutation: string; quotationType: string; gstPercent: number; serviceChargePercent: number; items: { itemName: string; qty: number; rate: number; amount: number }[]; total: number } {
    ...
    return {
      ...
      attention: String(data.attention || '').slice(0, 200),
      salutation: ['Dear Sir,', 'Dear Madam,', 'Dear Sir/Madam,'].includes(String(data.salutation)) ? String(data.salutation) : 'Dear Sir/Madam,',
      ...
      try {
        return await db.transaction(async (tx) => {
          ...
          const [result] = await tx.execute(sql`
            INSERT INTO quotations (quotation_no, quotation_date, quotation_thru, client_name, total_amount, status, remarks, created_by, quotation_type, gst_percent, service_charge_percent, subject, attention, salutation)
            VALUES (${quotationNo}, ${q.quotationDate}, ${q.quotationThru}, ${q.clientName}, ${q.total}, 'open', ${q.remarks}, ${createdBy}, ${q.quotationType}, ${q.gstPercent}, ${q.serviceChargePercent}, ${q.subject}, ${q.attention}, ${q.salutation})
          `) as any;
